...
String title="";
  ...
  Future<void> sendCustomNotification(String title,String description, SearchState state,UserModel admin,[String imageurl]) async {
    // final usernameRegex = r"(@\w*[a-zA-Z1-9])";
    // RegExp regExp = new RegExp(usernameRegex);
    // var status = regExp.hasMatch(description);
...
for(int i=0;i<state.userlist.length;i++) {
  await sendNotificationUser(title,description,state.userlist[i],admin);
}
  }
  ...
  Future<void> sendNotificationUser(String title,String description, UserModel user,UserModel admin) async {
    print("Send notification to: ${user.userName}");
