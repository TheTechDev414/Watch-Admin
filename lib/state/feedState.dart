...
class FeedState extends AppState {
  ...
  FeedModel _tweetToReplyModel; List<BlogModel> _blogs=List<BlogModel>();
  ...
  createNews(NewsModel model) {
    ...
    model.id=key;
    try {
      kDatabase.child('news').child(key).set(model.toJson());
      _newslist.add(model);

      notifyListeners();
    } catch (error) {
      cprint(error, errorIn: 'createNews');
    }
    isBusy = false;
    notifyListeners();
  }
  ...
  updateBlog(BlogModel model) {
    ///  Create tweet in [Firebase kDatabase]
    notifyListeners();

    try {
      kDatabase.child('blogs').child(model.key).set(model.toJson());
  ...
  deleteBlog(BlogModel model) {
    try {
      /// Delete tweet if it is in nested tweet detail page
      kDatabase.child('blogs').child(model.key).remove().then((_) {
        if(_blogs.contains(model)) {
          _blogs.remove(model);
          notifyListeners();
        }
        getBlogs();
      ...
      cprint(error, errorIn: 'deleteNews');
    }
  }
  deleteNews(NewsModel model) {
    try {
      /// Delete tweet if it is in nested tweet detail page
      kDatabase.child('news').child(model.id).remove().then((_) {
...
getNewsFromDatabase();
  ...
  Future<String> uploadnewsFile(File file) async {
    try {
      isBusy = true;
      notifyListeners();
      var storageReference = FirebaseStorage.instance
  ...
  updateNews(NewsModel model) async {

    await kDatabase.child('news').child(model.id).set(model.toJson());
